...
export function ChatForm({ company, tripId }: { company: string; tripId: string }) {
  ...
  return (
    <div className="flex flex-col h-screen max-w-2xl mx-auto bg-white dark:bg-slate-950 shadow-xl overflow-hidden relative">
      {/* Header */}
      <header className="bg-primary text-primary-foreground p-4 flex items-center shadow-md z-10">
        <Link href={`/${company}`} className="mr-4 hover:bg-white/10 p-2 rounded-full transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="font-semibold text-lg leading-tight">Assistente de Reserva</h1>
          <p className="text-primary-foreground/80 text-xs">Preencha seus dados para garantir a vaga</p>
        </div>
      </header>

      {/* Área de Mensagens */}
      <div className="flex-1 overflow-y-auto p-4 no-scrollbar bg-slate-50 dark:bg-slate-900">
        ...
        {isReviewing && (
          <motion.div
            ...
            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 mt-6 shadow-sm mb-4"
          >
        ...
        <div className="p-4 bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800">
          <div className="flex items-center space-x-2">
            {currentQuestion.type === 'select' ? (
              ...
              <div className="flex flex-1 items-center bg-slate-100 dark:bg-slate-800 rounded-full px-4 py-1 focus-within:ring-2 focus-within:ring-primary/50 transition-all border border-transparent focus-within:border-primary/50">
                <input
                ...
                <button
                  ...
                  className="bg-primary text-white p-2 rounded-full disabled:opacity-50 hover:bg-primary/90 transition-colors ml-2"
                >
